...
class PipelineResult:
    ...
    def model_results(self) -> dict[str, dict[str, Any]]:
        return {
            k: v
            for k, v in self.agent.record.last_success_by_name.items()
            if k.startswith("predict_")
        }

    @property
    def policy_result(self) -> dict[str, Any] | None:
        return self.agent.record.rag_result
...
class CreditRiskAIPipeline:
    ...
    Qwen policy RAG tool calling -> verified results + policy citations -> Qwen narrator.
    ...
    def __init__(
        ...
        narrator: QwenNarrator | None = None,
        ...
        self.narrator = narrator or QwenNarrator(settings=self.s)
    ...
    def _policy_context(
        ...
        """Small, raw-fact context Qwen may use to formulate the RAG query.

        This is NOT an ML feature payload. It is only policy-relevant application
        context, and RAG citations remain authoritative for policy statements.
        """
        allowed = {
            "company_name",
            "report_date",
        ...
        for key, extracted_value in extraction.raw_facts.items():
            if key in allowed:
                facts[key] = extracted_value.value
    ...
    def run_text(
        ...
        *,
        ...
        agent = self.agent.run(
            self._qwen_context(feature_context),
            policy_context=self._policy_context(extraction),
