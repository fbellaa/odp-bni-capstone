# V8 — Trusted Binding Architecture

V8 keeps Qwen as the real tool-calling agent, but removes model-feature transport
from the LLM. Qwen selects `predict_pd`, `predict_ews`, `predict_lgd`, and
`predict_pd_cluster`; Python then binds the exact deterministic `feature_context`
for the selected tool.

This prevents LLM-generated or cross-model feature payloads from reaching ML models.

Use the V8 Colab notebook:
`ml/notebooks/05_agentic_ai_end_to_end_v8_colab.ipynb`
...
document mapping, Qwen tool calling, SahabatAI grounded narration, and
